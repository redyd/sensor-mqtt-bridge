...
class Lmt84Sensor(TemperatureSensor):
    ...
    _SLOPE_MV_PER_C: float = -5.50
    _OFFSET_MV: float = 1035

    def __init__(self, channel: int = 0, reference_voltage: float = 3.3) -> None:
    ...
    def get_temperature(self) -> tuple[float, float]:
        raw = self._adc.value * self._reference_voltage * 1000  # convert ADC ratio to millivolts
